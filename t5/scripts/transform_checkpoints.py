...
flags.DEFINE_integer("autoensemble_size", 4,
                     "ensemble size for 'autoensemble'")
...
def main(_):
    ...
    if (FLAGS.operation == "average_last_n"
            and len(FLAGS.model_dirs_or_checkpoints) > 1):
        raise ValueError("Need only 1 directory for %s operation" %
                         FLAGS.operation)
    ...
    for path in FLAGS.model_dirs_or_checkpoints:
        if tf.io.gfile.isdir(path):
            ...
            if FLAGS.operation == "average_last_n":
                ckpt_paths = tf.io.gfile.glob(
                    os.path.join(path, "model.ckpt*index"))
                ...
                checkpoints.append(
                    checkpoint_state.all_model_checkpoint_paths[-1])
        else:
            if FLAGS.operation == "average_last_n":
                raise ValueError(
                    "need a directory while running %s operation" %
                    FLAGS.operation)
    ...
    if FLAGS.operation in ["ensemble", "average", "average_last_n"]:
        ...
        if len(checkpoints) != 1:
            raise ValueError("operation %s requires exactly one checkpoint" %
                             FLAGS.operation)
