...
def get_valid_eval_tasks(tasks, split):
    ...
    for task in tasks:
        ...
        if not task.metric_fns:
            logging.info("Task %s has no metric_fns; skipping eval.",
                         task.name)
            continue
...
def write_targets_and_examples(summary_dir, targets, examples):
  ...
  """
    ...
    for task in targets.keys():
        ...
        inputs_filename = os.path.join(summary_dir, "{}_inputs".format(task))
  ...
  """
    ...
    for task in tasks:
        ...
        for ex in tfds.as_numpy(ds):
            max_sequence_length["inputs"] = max(max_sequence_length["inputs"],
                                                len(ex["inputs"]))
            ...
            if "targets_plaintext" in ex:
                targets.append(
                    task.postprocess_fn(tf.compat.as_text(
                        ex["targets_plaintext"]),
                                        example=ex,
                                        is_target=True))
            else:
                targets.append(
                    task.postprocess_fn(tf.compat.as_text(ex["targets"]),
                                        example=ex,
                                        is_target=True))
...
def get_vocabulary(mixture_or_task_name=None):
    ...
    if not mixture_or_task_name:
        ...
        try:
            ...
            logging.warning(
                "Could not extract mixture/task name from gin config.")
    if mixture_or_task_name:
        ...
        if "inputs" in features and "targets" in features:
            return (features["inputs"].vocabulary,
...
def get_latest_checkpoint_from_dir(model_dir):
  ...
  """
    ...
    if ckpt is None:
        raise ValueError("No checkpoints found in model directory: %s" %
                         model_dir)
  ...
  **Note**: This only works for models checkpoints saved using Tensorflow.
  ...
  """
    def _get_closest_checkpoint(target_checkpoint):
        ...
        if not checkpoints:
            raise ValueError(
                "No checkpoint files found in {}".format(model_dir))
        closest = float("inf")
    ...
    if checkpoint_steps is None:
        if model_dir is None:
            raise ValueError(
                "checkpoint_steps and model_dir both cannot be None.")
