...
def compute_avg_glue(df, metric_names=None):
  ...
  """
    ...
    all_glue_tags = {
        k
        for k in metric_names.keys() if "glue" in k and "average" not in k
    }
    ...
    for tag in scores.keys():
        if tag not in metric_names:
            ...
            logging.warning(
                ...
                "Using tag as metric name.", tag)

    # Sort the tags in scores according to metric_names order
    sorted_tags = sorted(scores.keys(),
                         key=lambda x: list(metric_names.keys()).index(x))
...
def run_eval(
        ...
        predict_or_score_fn: Callable[[
            int, Any, Sequence[t5.data.Task], Sequence[Mapping[str, Mapping[
                str,
                str]]], Sequence[Mapping[str,
                                         tf.data.Dataset]], Union[None,
                                                                  Mapping[str,
                                                                          int]]
