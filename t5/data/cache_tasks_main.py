...
flags.DEFINE_string("output_cache_dir", None,
  ...
  """
    ...
    """
        ...
        logging.info("%s %s shards: %s", task.name, split,
                     ", ".join(["%s" % f for f in self.shards]))

    def _increment_counter(self, name):
        metrics.Metrics.counter(str("%s_%s" % (self._task.name, self._split)),
                                name).inc()
    ...
    def _emit_examples(self, shard):
        ...
        ds = self._task.source.get_dataset(split=self._split,
                                           shard=shard,
                                           shuffle=False)

        if self._max_input_examples:
            num_shard_examples = int(self._max_input_examples /
                                     len(self.shards))
  ...
  """
    ...
    def expand(self, pcoll):
        ...
        def _count_tokens(pcoll, feat):
            def _count(ex):
                if (feat in ex and isinstance(ex[feat], np.ndarray)
                        and ex[feat].dtype in (np.int32, np.int64)):
                    yield ("%s_tokens" % feat, int(sum(ex[feat] > 1)))

            return pcoll | "key_%s_toks" % feat >> beam.FlatMap(_count)

        token_counts = (
            [_count_tokens(pcoll, feat) for feat in self._output_features]
...
def run_pipeline(pipeline,
    ...
    for task_name in task_names:
        ...
        if not task.supports_caching:
            logging.info("Skipping task that does not support caching: '%s'",
                         task.name)
            continue
        ...
        for split in task.splits:
            ...
            pat = PreprocessTask(task, split, max_input_examples,
                                 modules_to_import)
...
def main(_):
    ...
    t5.data.add_global_cache_dirs([FLAGS.output_cache_dir] +
                                  FLAGS.tasks_additional_cache_dirs)
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        output_dirs = run_pipeline(pipeline, FLAGS.tasks,
                                   FLAGS.output_cache_dir,
                                   FLAGS.max_input_examples,
                                   FLAGS.excluded_tasks, FLAGS.module_import,
                                   FLAGS.overwrite)

    # TODO(adarob): Figure out a way to write these when each task completes.
    for output_dir in output_dirs:
        with tf.io.gfile.GFile(os.path.join(output_dir, "COMPLETED"),
                               "w") as f:
            f.write("")
