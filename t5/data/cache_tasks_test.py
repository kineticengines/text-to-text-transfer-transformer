...
class ProcessTaskBeamTest(test_utils.FakeTaskTest):
        ...
        for fname in expected_auxiliary_files:
            self.assertEqual(
                tf.io.gfile.GFile(os.path.join(
                    expected_task_dir,
                    fname)).read().replace('"num_shards": 2',
    ...
    def test_v3_pipeline(self):
        self.validate_pipeline("task_v3",
                               num_shards=1,
                               token_preprocessed=True)

    def test_v3_cache_before_tokenization_pipeline(self):
        self.validate_pipeline("task_v3_tokenized_postcache",
    ...
    def test_overwrite(self):
        with TestPipeline() as p:
            _ = cache_tasks_main.run_pipeline(p, ["uncached_task"],
