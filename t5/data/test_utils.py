...
def _assert_compare_to_fake_dataset(ds,
                                    split,
                                    features,
...
def verify_task_matches_fake_datasets(
    ...
    for split in splits:
        _assert_compare_to_fake_dataset(
            task.get_dataset(_SEQUENCE_LENGTH,
                             split,
...
def assert_dataset(dataset, expected):
  ...
  """
    ...
    def _compare_dict(actual_dict, expected_dict):
        _pyunit_proxy.assertEqual(set(actual_dict.keys()),
        ...
        for key, actual_value in actual_dict.items():
            ...
            np.testing.assert_array_equal(actual_value,
                                          _maybe_as_bytes(expected_dict[key]),
                                          key)
...
def assert_datasets_eq(dataset1, dataset2):
    ...
    def _compare_dict(dataset1, dataset2):
        ...
        for key, value1 in dataset1.items():
            ...
            np.testing.assert_array_equal(value1,
                                          _maybe_as_bytes(dataset2[key]), key)
...
def assert_datasets_neq(dataset1, dataset2):
    ...
    _pyunit_proxy.assertRaises(AssertionError, assert_datasets_eq, dataset1,
                               dataset2)
...
def get_fake_dataset(split, shuffle_files=False, seed=None):
    ...
    return tf.data.Dataset.from_generator(lambda: _FAKE_DATASET[split],
...
def test_token_preprocessor(dataset, output_features, sequence_length):
    ...
    def my_fn(ex):
        ...
        res["inputs"] = tf.where(tf.greater(inputs, 15),
                                 tf.constant(50, tf.int64), inputs)
        return res
...
def token_preprocessor_no_sequence_length(dataset, output_features):
    return test_token_preprocessor(dataset,
                                   output_features,
