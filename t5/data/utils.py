...
def dict_to_tfexample(ex):
    ...
    for k, v in ex.items():
        ...
        if len(t.shape) == 0:  # pylint:disable=g-explicit-length-test
            ...
            raise ValueError("Unsupported shape (%s) for '%s' value: %s" %
                             (t.shape, k, v))

        if t.dtype == tf.string and len(t.shape) <= 1:
            feature_dict[k] = tf.train.Feature(bytes_list=tf.train.BytesList(
                value=[tf.compat.as_bytes(t) for t in v]))
        elif t.dtype in (tf.int32, tf.int64) and len(t.shape) <= 1:
            feature_dict[k] = tf.train.Feature(int64_list=tf.train.Int64List(
                value=v))
        elif t.dtype in (tf.float32, tf.float64) and len(t.shape) <= 1:
            feature_dict[k] = tf.train.Feature(float_list=tf.train.FloatList(
                value=v))
        else:
            raise ValueError(
  ...
  """
    def map_without_seeds(fn):
        ...
        def wrapped_fn(ds, *args, **kargs):
            return ds.map(lambda arg: fn(arg, *args, **kargs),
    ...
    def map_with_seeds(fn):
        ...
        wrapped_fn.__signature__ = sig.replace(parameters=tuple(
            p for p in sig.parameters.values()
            if p.name not in ("seed", "seeds")))
        return wrapped_fn
