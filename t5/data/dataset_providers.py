  ...
  """
    ...
    def add(cls, name, provider_cls, *provider_args, **provider_kwargs):
        ...
        if name in cls._REGISTRY:
            raise ValueError("Attempting to register duplicate provider: %s" %
                             name)
    ...
    def get_dataset(cls,
                    ...
                    shuffle=True,
                    seed=None):
        return cls.get(name).get_dataset(sequence_length=sequence_length,
                                         split=split,
...
class DataSource(DatasetProviderBase):
  ...
  """
    ...
    def __init__(self,
        ...
        self._num_input_examples = (dict(num_input_examples) if
                                    num_input_examples is not None else None)
    ...
    def get_dataset(self,
                    ...
                    shard: Optional[str] = None) -> tf.data.Dataset:
...
class TfdsDataSource(DataSource):
    ...
    def __init__(self,
                 ...
                 splits: Optional[Union[Iterable[str], Mapping[str,
                                                               str]]] = None):
    ...
    def get_dataset(self,
                    ...
                    shard: Optional[str] = None) -> tf.data.Dataset:
        if shard:
            return self.tfds_dataset.load_shard(shard,
                                                shuffle_files=shuffle,
                                                seed=seed)
...
class FileDataSource(DataSource):
    ...
    """
        ...
        super().__init__(splits=split_to_filepattern.keys(),
...
class TextLineDataSource(FileDataSource):
    ...
    """
        ...
        super().__init__(read_file_fn=read_file_fn,
...
class TFExampleDataSource(FileDataSource):
    ...
    """
        ...
        super().__init__(read_file_fn=reader_cls,
                         split_to_filepattern=split_to_filepattern,
                         num_input_examples=num_input_examples)
    ...
    def get_dataset(self,
        ...
        ds = super().get_dataset(split=split,
                                 shuffle=shuffle,
                                 seed=seed,
                                 shard=shard)
        return ds.map(lambda pb: tf.io.parse_single_example(
            pb, self._feature_description),
                      num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
class TaskV3(DatasetProviderBase):
  ...
  """
    def __init__(self,
                 ...
                 metric_fns: Optional[Callable[..., Mapping[str,
                                                            float]]] = None,
    ...
    """
        ...
        if cache_step_idx > -1:
            for prep in preprocessors[:cache_step_idx]:
                if "sequence_length" in inspect.signature(
                        prep).parameters.keys():
                    raise ValueError(
    ...
    """
        ...
        def _ensure_no_eos(feat, v):
            ...
            with tf.control_dependencies([
                    tf.debugging.assert_none_equal(
                        ...
                        message=
                        "Feature '{feat}' unexpectedly contains EOS=1 token "
        ...
        if ensure_no_eos:
            dataset = dataset.map(
                lambda ex: {k: _ensure_no_eos(k, v)
                            for k, v in ex.items()},
                num_parallel_calls=tf.data.experimental.AUTOTUNE)
        return dataset
    ...
    def _trim_and_ensure_eos(
        ...
        return dataset.map(
            lambda ex: {k: _trim_and_append_eos(k, v)
                        for k, v in ex.items()},
            num_parallel_calls=tf.data.experimental.AUTOTUNE)
        ...
        if not self._cache_dir:
            ...
            for cache_dir in potential_cache_dirs:
                if tf.io.gfile.exists(os.path.join(cache_dir, "COMPLETED")):
                    ...
                    logging.info("'%s' is cached at %s.", self.name,
                                 self.cache_dir)
                    break
    ...
    def assert_cached(self) -> None:
        ...
        assert self.cache_dir, (
            "'%s' does not exist in any of the task cache directories" %
            self.name)

    def get_cached_stats(
            self,
            split: str = tfds.Split.TRAIN) -> Mapping[str, Union[int, float]]:
    ...
    """
        ...
        if use_cached:
            ...
            ds = self.source.get_dataset(split=split,
                                         shuffle=shuffle,
                                         seed=seed)
    ...
    def _get_cached_dataset(self,
        ...
        self.assert_cached()
        with tf.io.gfile.GFile(utils.get_info_path(self.cache_dir,
                                                   split)) as f:
        ...
        ds = tf.data.Dataset.list_files(
            "%s-*-of-*%d" % (utils.get_tfrecord_prefix(
                self.cache_dir, split), split_info["num_shards"]),
            shuffle=shuffle,
            seed=seed)
        ...
        ds = ds.map(lambda ex: tf.io.parse_single_example(ex, feature_desc),
                    num_parallel_calls=tf.data.experimental.AUTOTUNE)
        if self.get_cached_stats(
                split)["examples"] <= _MAX_EXAMPLES_TO_MEM_CACHE:
            ds = ds.cache()
        return ds
    ...
    def postprocess_fn(self, decoded_model_output: Any,
        ...
        if self._postprocess_fn:
            return self._postprocess_fn(decoded_model_output,
                                        **postprocess_kwargs)
  ...
  """
    def __init__(self,
        ...
        if isinstance(output_features, dict):
            ...
            output_features = {
                k: Feature(utils.get_default_vocabulary())
...
class TfdsTask(Task):
    ...
    """
        super().__init__(name,
                         source=TfdsDataSource(tfds_name=tfds_name,
    ...
    """
        ...
        super().__init__(name,
                         source=TextLineDataSource(
                             ...
                             num_input_examples=task_kwargs.pop(
                                 "num_input_examples", None)),
                         text_preprocessor=text_preprocessor,
                         metric_fns=metric_fns,
                         dataset_fn=None,
...
class Mixture(DatasetProviderBase):
    ...
    def tasks(self) -> Iterable[TaskV3]:
        ...
        return list(
            sorted(set(sum(sub_tasks, self._tasks)), key=lambda t: t.name))
    ...
    """
        ...
        for task in self.tasks:
            if split not in task.splits:
                logging.info("Task %s has no '%s' split, skipping.", task.name,
                             split)
                continue
            tasks.append(task)
        if not tasks:
        ...
        if (split == "train" and use_cached
            ...
            _log_mixing_proportions(tasks, datasets, rates, dataset,
...
class MixtureRegistry(DatasetProviderRegistry):
    ...
    def add(cls, name, tasks, default_rate=None):
        super(MixtureRegistry, cls).add(name, Mixture, name, tasks,
                                        default_rate)
